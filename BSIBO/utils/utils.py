...
def random_crop_torch(imgs, out=84):
    n, c, h, w = imgs.shape
    crop_max = h - out + 1
...
def random_crop_padding(imgs, out=84):
    B,C,H,W = imgs.shape
    assert H >= out and W >= out, "Image is too small to be cropped"
    t = random_crop_torch(imgs, out)
    padding_offset = H - out
    pad_left =  padding_offset // 2 
    pad_right = pad_left + 1 if padding_offset % 2 else pad_left
    assert out + pad_left + pad_right == H, "Pad distance error"
    # pad_imgs = np.pad(t, ((0, 0), (0, 0), (pad_left, pad_right), (pad_left, pad_right)),
    #    mode='constant', constant_values=1.)   # pad white
    pad_imgs = F.pad(t, (pad_left, pad_right, pad_left, pad_right),
       mode='constant', value=1.)   # pad white
    return pad_imgs
