...
class BSIBOSacAgent(object):
    ...
    def __init__(
        ...
        self.bisim = BisimMetric(obs_shape).to(device)
        self.bisim_target = BisimMetric(obs_shape).to(device)

        self.critic_target.load_state_dict(self.critic.state_dict())
        self.encoder_target.load_state_dict(self.encoder.state_dict())
        self.bisim_target.load_state_dict(self.bisim.state_dict())
        ...
        self.bisim_optimizer = torch.optim.Adam(
            self.bisim.parameters(), lr=bisim_lr, betas=(actor_beta, 0.999)
        )
    ...
    def update(self, replay_buffer, L, step):
        ...
        self.update_dbc_new(replay_buffer, L, step)
        self.update_bisim(replay_buffer, L, step)

    def update_bisim(self, replay_buffer, L, step):
        rst = replay_buffer.sample_by_action()
        if rst is None:
            # early stop
            return
        ...
        obses2, rewards2, next_obses2 = obses[perm], rewards[perm], next_obses[perm]

        t = torch.abs(rewards - rewards2) + self.bisim_coef * self.bisim(next_obses, next_obses2)
        pred = self.bisim(obses, obses2)
        loss = ((t - pred) * (t - pred)).mean()

        self.bisim_optimizer.zero_grad()
        loss.backward()
        self.bisim_optimizer.step()
        if step % self.log_interval == 0:
            L.log('train/bisim_loss', loss, step)
            L.log('train/bisim_target', t.mean(), step)

    def update_dbc_new(self, replay_buffer, L, step):
        """
        No transition model        
        ...
        bsm = self.bisim_target(obs_crop, obs2_crop)
