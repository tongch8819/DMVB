...
from BSIBO.ac import  Actor, Critic, LOG_FREQ
...
class BisimMultiViewAgentVariance(BisimAgent):
    ...
    def update_encoder(self, obs, action, reward, L, step):
        ...
        # 2. multi-view loss
        obs_aug = utils.random_crop_padding(obs, out=self.crop_size)
        s_aug = self.critic.encoder(obs_aug)  # s_aug: (N, D)
        N, D = s_aug.shape
        t = torch.zeros(size=(self.num_views, N, D))
        for i in range(self.num_views):
            obs_aug = utils.random_crop_padding(obs, out=self.crop_size)
            s_aug = self.critic.encoder(obs_aug)  # s_aug: (N, D)
            t[i, :, :] = s_aug
        s_bar = t.mean(dim=0)
        mv_loss = F.mse_loss(t, s_bar.repeat(self.num_views, 1, 1), reduction="mean")
